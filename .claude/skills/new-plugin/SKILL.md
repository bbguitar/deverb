---
name: new-plugin
description: Create a new iPlug2 plugin project by duplicating Deverb in the iPlug2OOS repository
---
...
2. **Check for existing projects:**
   - If other project folders exist besides Deverb, ask if user wants to continue with existing work or create new

3. **Run duplicate script:**
   ```bash
   ./duplicate.py Deverb [PluginName] [ManufacturerName]
   ```
...
5. **Ask about committing:**
   ...
   git commit -m "created [PluginName] based on Deverb"
   ```
...
```bash
./duplicate.py Deverb MySynth MyBrand
```
